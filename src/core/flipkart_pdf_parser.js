...
var PdfObjectParser_FK = {
        ...
        if (dummyObj instanceof Array) {
            ...
            for (var prp in dummyObj) {
                if (!(dummyObj[prp] instanceof Function))
                    objArr[prp] = this.deserializeObject(dummyObj[prp],xref);
            }
            return objArr;
        }
    ...
    deserializeLinearization:function(dummyLinearization,xref){
    ...
    serializeCatalog: function (catalog) {
        ...
        var serializeProperties = ["documentOutline", "metadata"];
